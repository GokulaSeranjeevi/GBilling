...
import com.skywings.gbilling.logic.EInvoiceLogic;
...
@Component
@Scope("prototype")
public class FrmEInvoice extends JFrame implements ActionListener, KeyListener {
	...
	private JButton btnEntryAdd, btnEntryClear, btnJson;

	private Color color1 = Color.decode("#003C3C");
	private Color color2 = Color.decode("#FFFFFF");
	private Color color3 = Color.decode("#194F4F");
	private Color color4 = Color.decode("#FFA500");
	...
	public FrmEInvoice() throws Exception {
...
//		txtTranNo.setText(salesLogic.getTranNo());
		 ...
		 * 
		 * cmbAcctName.addListItem(new ListItem(vendor.getAcctName(),
		 * vendor.getAcctCode()));
		 * 
		 ...
		 * 
		 * cmbSaleman.addListItem(new ListItem(sales.getOperNameS(),
		 * sales.getOperatorId()));
		 * 
		 * }
		 * 
		 ...
		 * 
		 * cmbSubProduct.addListItem(new ListItem(pro.getSubProName(),
		 * pro.getSubcode()));
		 * 
	...
	private void createListeners() {
		 ...
		 * 
		 * if (e.getKeyCode() == KeyEvent.VK_ENTER) { cmbAcctName.requestFocus(); }
		 * 
		 * } }); txtMRP.addKeyListener(new KeyAdapter() {
		 * 
		 * @Override public void keyTyped(KeyEvent e) { char c = e.getKeyChar(); if
		 * (!Character.isDigit(c)) { e.consume(); } } });
		 * 
		 * tblSales.addKeyListener(new KeyAdapter() {
		 * 
		 * @Override public void keyPressed(KeyEvent e) { if (e.getKeyChar() == 'd' ||
		 * e.getKeyChar() == 'D') {
		 * 
		 * int response = JOptionPane.showConfirmDialog(panelContent,
		 * "Are you sure to delete this row?", "Confirm Deletion",
		 * JOptionPane.YES_NO_OPTION); if (response == JOptionPane.YES_OPTION) {
		 * 
		 * int selectedRow = tblSales.getSelectedRow();
		 * 
		 * if (selectedRow != -1) { DefaultTableModel model = (DefaultTableModel)
		 * tblSales.getModel();
		 * 
		 * model.removeRow(selectedRow);
		 * 
		 * lstSales.remove(selectedRow); } } } else {
		 * JOptionPane.showMessageDialog(panelContent, "Deletion Cancelled ..!"); }
		 * 
		 * if (e.getKeyChar() == 'e' || e.getKeyChar() == 'E') { int selectedRow =
		 * tblSales.getSelectedRow();
		 * 
		 * // if (selectedRow != -1) { // DefaultTableModel model = (DefaultTableModel)
		 * tblSales.getModel(); // // String subProName = (String)
		 * model.getValueAt(selectedRow, 0); // double qty = (Double)
		 * model.getValueAt(selectedRow, 1); // String unitName = (String)
		 * model.getValueAt(selectedRow, 2); // double rate = (Double)
		 * model.getValueAt(selectedRow, 3); // double netAmount = (Double)
		 * model.getValueAt(selectedRow, 5); // double taxAmount = (Double)
		 * model.getValueAt(selectedRow, 6); // double grossAmount = (Double)
		 * model.getValueAt(selectedRow, 7); // //
		 * cmbSubProduct.setSelectedItem(subProName); //
		 * txtQty.setText(String.valueOf(qty)); // cmbUnit.setSelectedItem(unitName); //
		 ...
		 * "Apology to You, Now This Option Not Enabled ..!"); }
		 * 
		 * }
		 * 
		 * });
...
//		tblView();
	...
	private void tblView() {
		...
		lstSalesView = einvoiceLogic.getEinvoice();
		...
		if (tblView == null) {
			...
			scrView.setBounds(30, 200, 1850, 600);
			scrView.getViewport().setBackground(color2);
			scrView.setVisible(true);

			btnJson = new JButton("Create JSon");
			btnJson.setBounds(scrView.getX() + 800, scrView.getY() + scrView.getHeight() + 30, 180, 30);
			btnJson.setBackground(color7);
			btnJson.setForeground(color2);
			btnJson.setVisible(true);
//			btnJson.setMnemonic(KeyEvent.VK_C);
			btnJson.addKeyListener(this);
			btnJson.addActionListener(this);

			panelViewDetail.add(scrView);
			panelViewDetail.add(btnJson);
		}
	}
...
//		scrPane.revalidate();	
	...
	private java.awt.Component panelButtonInialize() {
		...
		panelButton.setBounds(0, panelView.getY() + panelView.getHeight() - 15, 1000, 40);
...
//		panelButton.setBorder(BorderFactory.createEtchedBorder(color3, color3));
	...
	private java.awt.Component panelDetailInitialize() {
		...
		lblCallMnuHead = new JLabel("E-INVOICE GENERATION");
		lblCallMnuHead.setBounds(10, -10, 170, 50);
		// lblHeading.setBounds(panelDetail.getWidth() / 2, panelDetail.getY() / 2, F20,
		// 20);
...
//			panelContent.setEnabled(false);
	...
	private void addFocusListeners() {
		 ...
		 * 
		 * @Override public void focusLost(FocusEvent e) { formatTextField(e);
		 * 
		 * } });
		 * 
		 ...
		 * 
		 * String text = txtMRP.getText(); if (!text.isEmpty()) { double value =
		 * Double.parseDouble(text); txtMRP.setText(nf.format(value)); } } });
		 * 
